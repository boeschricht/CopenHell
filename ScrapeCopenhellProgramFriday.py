...
PLAYLIST_NAME = "Copenhell Friday"
TRACKS_PER_ARTIST = 2
...
def get_top_tracks(token, band_name):
    ...
    # Slå artist-entity op for at få eksakt ID
    r = requests.get(
    ...
    # Søg tracks og filtrer på eksakt primær artist-ID
    r2 = requests.get(
        "https://api.spotify.com/v1/search",
...
def main():
    bands = scrape_friday_bands()
    print(f"Bands på fredag ({len(bands)} stk): {', '.join(bands)}\n")
    ...
    print(f"Bands fundet på Spotify: {len(all_uris)} tracks fra {len(bands)} bands")
